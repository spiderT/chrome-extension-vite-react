...
export default function App() {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>, index: number, type: string) => {
    rules[index][type] = e.target.value?.trim();
    setRule([...rules]);
    ...
    <div className="http-interceptor-wrap">
      <div className="all-switch">
        <Switch
          ...
          defaultChecked={window.setting?.ajaxInterceptorSwitchOn}
        />
      </div>
      <Collapse>
        {rules?.map((item, index) => (
            ...
            header={
              <div className="panel-header" onClick={(e) => e.stopPropagation()}>
                ...
                <Switch
                  ...
                  onChange={(v) => handleSwitch(v, index, 'switchOn')}
                  defaultChecked={item.switchOn}
                />
          ...
          >
            <TextArea
              rows={6}
              placeholder="mock接口返回值"
              onChange={(e) => handleChange(e, index, 'response')}
